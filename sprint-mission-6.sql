...
SELECT *
FROM orders
WHERE (date + time) >= '2025-03-12 00:00:00'
  AND (date + time) < '2025-03-12 12:00:00'
ORDER BY time ASC;
...
SELECT pizza_id, SUM(od.quantity * p.price) AS total_revenue
...
SELECT o.date, COUNT(DISTINCT o.id) AS order_count, SUM(quantity) AS total_quantity
FROM order_details AS od
...
SELECT p.size AS pizza_size, SUM(od.quantity * p.price) AS total_revenu
FROM order_details AS od
INNER JOIN pizzas AS p ON od.pizza_id = p.id
GROUP BY p.size
...
SELECT pt.name AS pizza_name, SUM(od.quantity * p.price) AS total_revenu
FROM order_details AS od
INNER JOIN pizzas AS p ON od.pizza_id = p.id
INNER JOIN pizza_types AS pt ON p.type_id = pt.id
GROUP BY pt.name
